...
#define COUNT_BANDS (sizeof(band_colors) / sizeof(band_colors[0]))
    ...
    // Try read all at once version
#if 1
    Serial.println("\n*** readRectOneShot ***");
    memset(pixel_data, 0, sizeof(pixel_data));

    tft.readRectOneShot(BAND_START_X, BAND_START_Y, BAND_WIDTH * 8, BAND_HEIGHT, pixel_data);

    MemoryHexDump(Serial, pixel_data, BAND_WIDTH * 8 * 2 + 16, true, "\nColor bars FlexIO:\n");

    MemoryHexDump(Serial, &pixel_data[(BAND_WIDTH * 8 * BAND_HEIGHT) / 2], BAND_WIDTH * 8 * 2 + 16, true, "\nCenter FlexIO:\n");

    uint16_t *ppixel = pixel_data;
    tft.writeRect(BAND_START_X, BAND_START_Y + 2 * BAND_HEIGHT + 5, BAND_WIDTH * 8, BAND_HEIGHT, pixel_data);
    for (uint8_t y = 0; y < BAND_HEIGHT; y++) {
        uint16_t error_count = 0;
        for (uint8_t band = 0; band < COUNT_BANDS; band++) {
            for (uint8_t ix = 0; ix < BAND_WIDTH; ix++) {
                if (*ppixel != band_colors[band]) {
                    error_count++;
                    if (error_count == 1) {
                        Serial.printf("%u - %u %u: %x != %x", y, band, ix, *ppixel, band_colors[band]);
                    }
                }
                ppixel++;
            }
        }
        if (error_count) Serial.printf(" errors: %u\n", error_count);
    }
    ...
    tft.readRectOneShot(0, 0, 50, 50, pixel_data);
    ...
    tft.readRectOneShot(85, 65, 50, 50, pixel_data);
